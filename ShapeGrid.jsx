/* ShapeGrid — animated tile grid canvas (React Bits), adapted: no CSS file, React from global. */
...
function ShapeGrid({
  ...
  useEffect(() => {
    ...
    const updateAnimation = () => {
      ...
      else if (direction === 'left') gridOffset.current.x = (gridOffset.current.x + effectiveSpeed + wrapX) % wrapX;
      else if (direction === 'up') gridOffset.current.y = (gridOffset.current.y + effectiveSpeed + wrapY) % wrapY;
      else if (direction === 'down') gridOffset.current.y = (gridOffset.current.y - effectiveSpeed + wrapY) % wrapY;
    ...
    const handleMouseLeave = () => { pushTrail(); hoveredSquare.current = null; };
    ...
    const tryStart = () => { if (isVisible && isPageVisible && !requestRef.current) requestRef.current = requestAnimationFrame(updateAnimation); };
    const tryStop = () => { if (requestRef.current) { cancelAnimationFrame(requestRef.current); requestRef.current = null; } };
  ...
  return <canvas ref={canvasRef} style={{ width: '100%', height: '100%', display: 'block', border: 'none', ...style }} />;
}

window.ShapeGrid = ShapeGrid;
module.exports = { ShapeGrid };
